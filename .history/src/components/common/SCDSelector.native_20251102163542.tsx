...
const PickerModal = ({ visible, onClose, options, labelKey = 'name', onSelect, selectedId }) => {
  return (
        ...
        <View style={styles.modalContent}>
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                <Text style={[styles.optionText, String(item?.id) === String(selectedId) && styles.optionSelected]}>
...
const SCDSelector = ({ formik, showSchool = true, showClass = true, showDivision = true }) => {
  ...
  const teacherAllocatedClasses = useMemo(() => currentUser?.allocatedClasses || [], [currentUser]);
  ...
  const filteredClasses = useMemo(() => {
    ...
    if (isTeacher && teacherAllocatedClasses.length > 0) {
      ...
      return classes.filter((c) => allocatedClassIds.includes(c?.id ?? c?.schoolClassId ?? c?.classId));
    }
  ...
  const filteredDivisions = useMemo(() => {
    ...
    if (isTeacher && teacherAllocatedClasses.length > 0) {
      ...
      return divisions.filter((d) => allocatedDivisionIds.includes(d?.id ?? d?.divisionId));
    }
    if (currentSchoolId) {
      return divisions.filter((d) => {
        const schoolKey = d?.schoolId ?? d?.schoolBranchId ?? d?.schoolbranchId ?? null;
  ...
  const classValue = (filteredClasses || []).find((c) => String(c?.id ?? c?.schoolClassId ?? c?.classId) === String(values?.classId)) || null;
  const divisionValue = (filteredDivisions || []).find((d) => String(d?.id ?? d?.divisionId) === String(values?.divisionId)) || null;
    ...
    <View style={styles.container}>
      ...
      {!isTeacher && showSchool && (
        <View style={styles.fieldRow}>
          ...
          <TouchableOpacity style={styles.input} onPress={() => setShowSchoolModal(true)}>
            <Text style={styles.inputText}>{schoolValue?.name ?? 'Select school'}</Text>
          </TouchableOpacity>
          {touched?.schoolId && errors?.schoolId && <Text style={styles.errorText}>{errors.schoolId}</Text>}
        </View>
      )}
      ...
      {showClass && (
        <View style={styles.fieldRow}>
          ...
          <TouchableOpacity style={styles.input} onPress={() => setShowClassModal(true)}>
            <Text style={styles.inputText}>{classValue?.name ?? 'Select class'}</Text>
          </TouchableOpacity>
          {touched?.classId && errors?.classId && <Text style={styles.errorText}>{errors.classId}</Text>}
        </View>
      )}
      ...
      {showDivision && (
        <View style={styles.fieldRow}>
          ...
          <TouchableOpacity style={styles.input} onPress={() => setShowDivisionModal(true)} disabled={loading || (!isTeacher && !values?.schoolId && !values?.classId)}>
            <Text style={styles.inputText}>{divisionValue?.name ?? 'Select division'}</Text>
          </TouchableOpacity>
          {touched?.divisionId && errors?.divisionId && <Text style={styles.errorText}>{errors.divisionId}</Text>}
        </View>
      )}
